...
"""
Description: Visualise the output side algorithm in the convolution of two generated signal waveforms.
Author: Jensen Benard
Date Created: July 6, 2023
Date Modified: July 6, 2023
...
def compute_step(input_y, impulse_y, prev_output_y):
    """Return the output side computation algorithm for
    a single step.
...
def main():
    ...
    # Initialiase axis for each signal for plotting.
    ...
    # Set axis limits for each signal based on signal values.
    ...
    # Add an index line for each signal required show current index.
    ...
    # Update input and impulse signal axis once since they do not change.
    ...
    for output_index, output_x, in enumerate(output_signal.x_points):
        for impulse_index, impulse_x in enumerate(impulse_signal.x_points):
            index_difference = output_index - impulse_index
            if index_difference >= 0 and index_difference <= impulse_signal.len - 1:
                step += 1
                output_signal.y_points[output_index] = compute_step(input_signal.y_points[index_difference],
                                               impulse_signal.y_points[impulse_index],
                                               output_signal.y_points[output_index])
            
            # Update output signal axis curve with calculated values.
            output_signal_axis.update_signal_line()

            # Update index line position and label with new index.
            input_signal_axis.update_index_line(f"Input index: {impulse_index}", index_difference)
            ...
            # Update the y limits of the output signal axis to show the updated curve.
